...
import { imageToDataURL } from '../utils/cardHelpers'
...
export function DuelProvider({ children }) {
  ...
  // ── Graveyard ─────────────────────────────────────────
  const [playerGY,   setPlayerGY]   = useState([])  // array of card objects
  ...
  const executeAction = useCallback((actionId) => {
    ...
    switch (actionId) {
      ...
      case 'set-monster': {
        ...
        const snap  = selectedCard          // capture before clear
        const rawImg = snap.card?.url || snap.card?.card_images?.[0]?.image_url || ''
        const displayUrl = rawImg ? `https://corsproxy.io/?url=${encodeURIComponent(rawImg)}` : ''

        // 1. Remove from hand immediately (visual response)
        ...
        // 2. Place on board after short delay (gives sense of travel)
        setTimeout(() => {
          setOccupiedZones(prev => ({
            ...prev,
            [zk]: { card: snap.card, dataUrl: displayUrl, position: isSet ? 'defense' : 'attack', faceDown: isSet, summonedThisTurn: true }
          }))
          // 3. Background fetch for real dataUrl (sobel glow FX)
          if (rawImg && !isSet) {
      ...
      case 'set-trap': {
        ...
        setTimeout(() => {
          setOccupiedZones(prev => ({
            ...prev,
            [zk]: { card: snap.card, dataUrl: displayUrl, position: 'spell', faceDown, summonedThisTurn: true }
          }))
          if (rawImg && !faceDown) {
            imageToDataURL(rawImg)
              .then(dataUrl => dataUrl && setOccupiedZones(prev => {
  ...
  return (
    <DuelContext.Provider value={{
      ...
      occupiedZones, placeCardInZone, sendToGraveyard,
      playerGY, opponentGY,
